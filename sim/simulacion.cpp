...
void Simulacion::movimiento_particulas() {
    for (int i = 0; i < num_particulas; ++i) {
        particulas.posicion[i] = calculadora.actualizar_posicion(particulas.posicion[i], particulas.gradiente[i],
                                                                 particulas.aceleracion[i]);
        particulas.gradiente[i] = calculadora.actualizar_gradiente(particulas.gradiente[i],
                                                                   particulas.aceleracion[i]);
        particulas.velocidad[i] = calculadora.actualizar_velocidad(particulas.gradiente[i],
                                                                   particulas.aceleracion[i]);
    }
}
